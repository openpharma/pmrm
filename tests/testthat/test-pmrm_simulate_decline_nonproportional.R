test_that("pmrm_simulate_decline_nonproportional() without covariates", {
  data <- pmrm_simulate_decline_nonproportional(gamma = numeric(0L))
  expect_true(tibble::is_tibble(data))
  expect_false(any(grepl("^w_", colnames(data))))
})

test_that("pmrm_simulate_decline_nonproportional() with covariates", {
  set.seed(0L)
  I <- 300L
  J <- 5L
