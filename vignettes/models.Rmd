...
The decline models characterize therapeutic benefit as a pointwise reduction (relative to the control arm) in the decline of patient health on the clinical outcome scale.

## The non-proportional decline model

The non-proprtional decline model defines the expected value $\mu_{ij}$ as follows:
...
where $b(i)$ is the study arm of patient $i$, and $\beta_{kj}$ is the reduction of decline in study arm $k$ ($k = 1, \ldots, K$) relative to the control arm $k = 1$.
...
The proportional decline model is the same as the non-proportional variant except the treatment effects $\beta_1, \ldots, \beta_K$ no longer depend on the visit.
