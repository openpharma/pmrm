...
- initial:
  ...
  - `gamma`: a vector with `V` elements, where `V` is the number of
    ...
    `fit <- pmrm_model_decline(...)`) and then check
  ...
  test model (e.g. `fit <- pmrm_model_decline(...)`) and then extracting
  `fit$initial` or `fit$final`.
...
and `pmrm_model_slowing()`. It has the following named elements:
