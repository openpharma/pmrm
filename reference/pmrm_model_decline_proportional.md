...
Fit the proportional decline model to a clinical dataset on a
progressive disease.
...
- initial:
  ...
  - `gamma`: a vector with `V` elements, where `V` is the number of
    ...
    `fit <- pmrm_modepmrm_model_decline_proportionall_decline(...)`) and
    then check `ncol(fit$constants$W)`.
  ...
  test model (e.g. `fit <- pmrm_model_decline_proportional(...)`) and
...
``` r
  ...
  str(fit$estimates)
#> List of 9
#>  $ alpha : Named num [1:5] 0.000599 0.699851 1.036801 1.386083 1.642306
#>   ..- attr(*, "names")= chr [1:5] "1" "2" "3" "4" ...
#>  $ theta : num [1:2] 0.155 0.222
#>  $ gamma : num [1:2] 1.01 1.96
#>  $ phi   : num [1:5] 0.00705 -0.02844 0.02153 -0.02724 0.00774
#>  $ rho   : num [1:10] -0.00549 -0.06436 0.03036 -0.10029 -0.08156 ...
#>  $ beta  : num [1:3] 0 0.155 0.222
#>  $ sigma : num [1:5] 1.007 0.972 1.022 0.973 1.008
#>  $ Lambda: num [1:5, 1:5] 1 -0.00549 -0.06402 0.03021 -0.09859 ...
#>  $ Sigma : num [1:5, 1:5] 1.0142 -0.00538 -0.06587 0.02961 -0.10006 ...
  names(fit)
